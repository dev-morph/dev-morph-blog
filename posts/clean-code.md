...
사실 처음 취직했을 때에 클린코드에 대해 들었을 때에는 **"당연히 그렇게 짜면 좋겠지. 근데, 어떤게 클린 코드지? 나는 공식문서보면서 코딩하고 있으면 클린한 코드가 아닌가?"** 라는 생각을 했습니다. 필독서라는 **로버트마틴의 CleanCode**라는 책도 읽어 보고 몇 가지는 실제 프로젝트에 반영해 보기도 했지만 크게 와닿지는 않았습니다. 그래서 어느새 제가 개발자란 직업을 가지게 된 지 만 2년이 넘어 3년차가 된 기념으로, 지금 시점에서 제가 생각하는 클린 코드는 무엇인지, 코드를 클린 하게 짜기 위해 어떤 노력하고 있는지 실제 제가 리팩토링 한 리액트 코드와 함께 정리해 보고자 합니다.
...
-   **원하는 부분을 빨리 찾을 수 있어야 합니다. ➡ 즉, 가독성이 좋아야 하며 유지보수가 용이해야 합니다.**  
    잦은 요구사항 변경 때문에, 다들 한 번쯤은 골머리를 앓아보셨을 것입니다. 그런데 변경된 요구사항을 반영하기 위해 찾아 들어간 파일이 jsx 부분만 몇 백 줄이 넘어간다면 어떨까요? 거기다가 함수명이 clickHandler, btnClickHandler와 같이 도대체 무슨 역할을 하는지 알 수 없다면...? 당연히 변경사항이 생길 때마다 추가 개발 시간이 많이 소요될 것입니다.
-   **불필요한 부분은 감추고 핵심로직만 노출 되어야 합니다.**  
    커스텀 컴포넌트나 훅을 만들 때, 불필요한 부분은 감추고 핵심 로직만 노출될 수 있도록 해야 합니다. 불필요한 부분이 너무 많이 노출되어 있다면 사용할 때 혼란을 겪을 것이고, 너무 많이 감췄다면 어떤 기능을 하는지 정확히 알기 힘들 것입니다.

말은 좀 길었지만, 결국 가독성 좋게(네이밍 규칙 잘 지키고) 코드를 짜고, 핵심 로직은 노출, 불필요한 부분은 숨기기만 해도 클린 한 코드에 한걸음 다가갈 수 있을 것 같습니다.
...
[##_Image|kage@crAc72/btsDJZhHAiy/a6lxqnkkcrjJpOD5F0F3YK/img.png|CDM|1.3|{"originWidth":2100,"originHeight":840,"style":"alignCenter","caption":"포스트 카드 사진"}_##]
...
제 나름대로 클래스명을 명확하게 짓기 위해 노력했지만, 이 코드를 작성하고 몇 달 후에 마주한다면 처음부터 읽어가며 이 코드의 어떤 부분이 무엇을 의미하는지 고민하게 될 것입니다. ~사실 몇 달 전만해도 저는 위의 코드가 그렇게 나쁘지 않다고 생각했을 것 입니다.~ 그럼 바로 제가 어떻게 수정했는지 코드로 보겠습니다.

## **\[수정 후 코드\]**

제가 수정 해 본 코드는 아래와 같습니다. 훨씬 명확해졌지 않나요?  
(참고로 @morphlib는 제가 만든 컴포넌트들이 담겨있는 dir shortcut입니다. 요즘은 이런식으로 관리하는게 명확하고 보기 좋더라구요.)
...
이 훅을 사용할 때 저는 앞서 언급한 위 3가지에 대해서만 집중하면 되고 내부 로직에 대해서는 더 이상 신경 쓰지 않아도 됩니다. 그저 submitHandler에서 유효성 검사를 통과했으면(valid 하다면) formData를 서버에 보내주면 되고, 아니라면 return 해주면 됩니다.

물론, useForm을 더 추상화하여 코드 라인을 줄일 수도 있습니다. 예를 들어, useForm() 호출만 하면 해당 페이지 form에 대해 유효성 검사를 진행하는 식으로요. 하지만 이렇게 되면 각 input이 어떤 로직으로 유효성 검사가 진행되는지, 현재 form이 검사를 통과했는지 등 앞서 말씀드린 핵심 로직을 알기 힘들어집니다. 그리고 이 상황에선 애초에 useForm이 무엇을 하는지조차 불분명 해집니다.

따라서 **무조건 코드를 줄이기 위해 전부** **추상화하는 것이 아니라, 적절하게 핵심 로직은 노출하고 불필요한 내부로 직은** **추상화**해야 하고, 이것이 우리가 코드를 좀 더 클린 하게 짤 수 있는 방법이라고 생각합니다. 물론 네이밍을 명확하게 짓는 것과 SRP와 같은 부분은 너무 중요하지만, 너무 많이 들어보셨을 부분들이라 이번 포스트에서는 크게 다루지 않기로 결정했습니다.
